...
const CreativeView = ({ chunk, onSubmit }: { chunk: Chunk, onSubmit: (res: FeedbackResult) => void }) => {
    ...
    const baseLang = context?.state.user?.baseLanguage ?? 'es';
    ...
    const handleCheck = async () => {
        ...
        try {
            ...
            const result = await AITutorService.evaluateCreative(input, chunk, undefined, baseLang);
    ...
    return (
        <div className="flex flex-col h-full">
            <h3 className="text-purple-600 font-bold mb-2 text-center uppercase tracking-widest text-sm flex justify-center gap-2"><PenTool size={16} /> {t.creativeMode}</h3>
            <p className="text-center text-gray-500 mb-6 text-sm">{t.creativeInstruction}</p>
            ...
            <textarea
                ...
                placeholder={AITutorService.getApiKey() ? t.creativeAIPlaceholder : t.creativeLocalPlaceholder}
                rows={3}
                disabled={isLoading}
            />
            ...
            <button onClick={handleCheck} disabled={!input.trim() || isLoading} className="w-full bg-purple-600 text-white py-3 rounded-xl font-bold disabled:opacity-50 shadow-lg hover:bg-purple-700 transition-colors flex justify-center items-center">
                {isLoading ? <><Loader2 className="animate-spin mr-2" /> {t.evaluating}</> : t.validate}
            </button>
        </div>
    );
};
