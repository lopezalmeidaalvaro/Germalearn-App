...
const WriteView = ({ chunk, onSubmit }: { chunk: Chunk, onSubmit: (res: FeedbackResult) => void }) => {
    ...
    return (
        <div className="flex flex-col h-full">
            <h3 className="text-gray-500 font-bold mb-6 text-center uppercase tracking-widest text-sm">Traduce exactamente</h3>
            ...
            <button onClick={handleSubmit} disabled={!input.trim()} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold disabled:opacity-50 shadow-lg">Comprobar</button>
        </div>
    );
};
